...
export default async function handler(req, res) {
	...
	verifier.setNickname(req.body.nickname);
	let errors = verifier.getErrors();
	if(errors.length) {
		res.send({error: errors[0]});
	}
	await PokemonCommands.setNickname(verifier.userID, verifier.uniquePokemonID, verifier.nickname);
	res.send({message: "Your nickname has been changed."});
}